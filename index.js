...
document.getElementById('searchButton').addEventListener('click', function() {
    ...
    let validLocations = ['مجمع الملك فيصل الطبي', 'جامعة الطائف', 'مطار الطائف الدولي', 'متحف قصر شبرا الاقليمي'];

    // Check if the input is a valid location
    if (validLocations.includes(searchValue)) {
        // Call the API to get parking status data
        fetch(`http://127.0.0.1:5000/parking-status?location=${searchValue}`)
            .then(response => response.json())
            .then(data => {
                // Update the UI with the returned data
                document.getElementById('available-parking').innerText = data.available_parking;
                document.getElementById('unavailable-parking').innerText = data.unavailable_parking;
                document.getElementById('total-parking').innerText = data.total_parking;

                // Scroll to the location section
                const locationSection = document.getElementById(searchValue);
        ...
        // Alert if the location is not valid
        alert("الموقع غير موجود. يرجى المحاولة مرة أخرى.");
    }
});
